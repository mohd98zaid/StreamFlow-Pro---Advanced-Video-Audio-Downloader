...
class DownloadQueue:
    ...
    def get_by_status(self, status: str) -> List[DownloadItem]:
...
class DownloadStats:
    ...
    def add_download(self, item: DownloadItem) -> None:
        """Update statistics with completed download"""
        ...
        # Track channels
        if item.channel:
            self.channels[item.channel] = self.channels.get(item.channel, 0) + 1
...
class FormatPreset:
    """Predefined format configuration"""
    
    def __init__(self, name: str, download_type: str, quality: str = "",
                 format_type: str = "", embed_thumbnail: bool = True,
                 embed_metadata: bool = True, embed_subtitles: bool = False):
        self.name = name
        self.download_type = download_type
        self.quality = quality
